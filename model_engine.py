# model_engine.py
"""
Multi-Model Engine for Indian Toxic Comment Detection.
Integrates FastText Subword Embeddings, Multilingual TF-IDF, Logistic Regression, Random Forest,
and Hybrid Lexicon-Sensitive Inference.
"""
...
class IndiToxModelEngine:
    def __init__(self):
        ...
        # Cache for models keyed by (feature_mode, model_type)
        self._models_cache: Dict[str, Dict[str, Any]] = {
            "tfidf": {},
            "fasttext": {}
        }

    def train_fasttext(self, sentences: List[List[str]], vector_size: int = 100, epochs: int = 15) -> FastText:
        """Train FastText subword model on tokenized sentences."""
        ft = FastText(
            sentences=sentences,
    ...
    def train_tfidf(self, texts: List[str]) -> Tuple[TfidfVectorizer, Any]:
        """Train character + word ngram TF-IDF vectorizer (optimal for Indic and Hinglish)."""
        vec = TfidfVectorizer(
            ngram_range=(1, 3),
            max_features=10000,
            analyzer="word",
    ...
    def train_models(
        ...
        """Train Logistic Regression (OvR) and Random Forest classifiers."""
    ...
    def predict_hybrid(
        ...
        """
        Inference combining ML model probabilities with Indic toxic lexicon sensitivity.
        Guarantees zero-day vulgarity detection and calibrated probabilities.
        """
        ...
        for i, col in enumerate(LABEL_COLS):
            ...
            # Blended score: max of ML and Lexicon with smooth dampening
            combined_p = min(1.0, max(ml_p, lex_p, 0.6 * ml_p + 0.5 * lex_p))
            final_probs[col] = float(np.round(combined_p, 4))
            final_labels[col] = 1 if combined_p >= 0.45 else 0

        # Any toxicity summary
        is_toxic = any(v == 1 for v in final_labels.values())
        overall_score = float(max(final_probs.values())) if final_probs else 0.0
